...
- Exception: after any completed collector-related change, including changes under `conf/collector*.toml`, `conf/collector_deploy_*.ps1`, collector UI/API code, or `src/core/collector.rs`, deploy to Windows with `pwsh -ExecutionPolicy Bypass -File scripts/deploy_windows.ps1` so the local collector process loads the latest behavior.
