...
一个可执行文件 `auto_sync` 同时承载调度器、文件监听、Web 服务，并在有显示环境时打开桌面窗口；控制功能统一由 Flutter/Web UI 与 HTTP API 提供。支持 Linux（含无头 NAS / OpenWrt）与 Windows，支持局域网多机自动发现与跨机原生 TCP 传输。
...
curl -s -X POST http://127.0.0.1:18765/api/sync-now
```
...
5. **展示与控制层**：Flutter 桌面窗口与 Web UI 展示配置、状态、进度；后台按 `status_log_interval_secs` 输出状态日志；HTTP API 提供脚本化控制。
...
入口在 `src/bin/auto_sync.rs`：主进程负责单实例锁、可选提权、调度线程、Web 服务与桌面启动。
...
**`sync.zfs_diff` 开关的完整语义** — 这是一个性能/信任边界开关，决定 Full / Compare / fallback reconcile 是否允许用 `zfs diff` 的“变化路径快路径”。它不是 ZFS snapshot 总开关：只要 snapshot backend 选中 ZFS，源侧仍会创建 snapshot 作为稳定读视图；关闭 `zfs_diff` 只是不再相信 baseline diff 来裁剪对账范围。

| 场景 | `zfs_diff = true`（默认） | `zfs_diff = false` |
| --- | --- | --- |
| 适用前提 | 只有源/目标是本机 ZFS 数据集，且有可用 baseline snapshot 时才启用。Full/Compare 需要 source + destination 两侧 baseline；源侧增量快路径只需要 source baseline。前提缺失自动回退。 | 始终跳过 `zfs diff` 快路径。 |
| 调度 Incremental / 手动 Incremental | **首选 event_log 路径**：重放该目标自上次 verified cycle 以来的 watcher/USN 事件路径。只要事件链完整，开关开/关都不改变“按事件路径增量同步”的语义。 | 同左：只用事件日志路径做增量。 |
| Incremental fallback | 如果 event_log 不可用、出现 `rescan_required`、目标从未 verified、或事件路径无法安全表达，就进入 reconcile。此时若 source baseline 可用，可用 source `zfs diff` 得到变化路径，只同步这些路径；失败再回退全树。 | 进入 reconcile 后直接走完整对账：并行扫描源树和目标树，再只修差异。 |
...
| Repair（Compare 后同步差异） | 完整报告时只同步报告里的差异路径，不依赖 `zfs diff`；报告被截断时升级为 Full，此时可走 Full 的 `zfs diff` 快路径。 | 完整报告时同左；截断升级 Full 后走完整双树扫描。 |
| 目标侧漂移检测 | Full/Compare 的双侧 diff 会把 destination baseline→当前目标的变化也纳入并集，因此能发现目标端被手工改动、删除或新增的路径。源侧-only incremental 不刷新目标 baseline，避免把目标漂移“洗进”新基准。 | Full/Compare 每次都扫完整目标树，天然能发现目标漂移；Incremental 仍只看事件路径，不主动验证整棵目标树。 |
| baseline 刷新 | 成功的全树级验证路径会刷新 source/destination baseline；只看源侧变化的 zfs-diff incremental 只推进 source baseline、保留 destination baseline。刷新前会用 `zfs diff` 交叉检查外部写入，发现污染则保留旧 baseline 或回退。 | 不使用 baseline 来裁剪 Full/Compare；相关 baseline 可被保留/清理，但不会作为快路径依据。 |
| 正确性边界 | 快路径依赖 baseline invariant：上一轮 verified 时源/目标全树一致，且期间未变化路径仍可信。适合正常连续运行、有可靠 snapshot 的大树。 | 不依赖 baseline 信任；适合怀疑 baseline、磁盘事故后、手工大量改目标端后先做一次保守对账。 |
| 性能 | 对百万级 ZFS 树，Full/Compare/fallback reconcile 的耗时通常与“变化路径数量”相关，差异少时可从分钟级降到秒级；但需要保留和维护 baseline snapshot。 | Full/Compare/fallback reconcile 耗时与“全树文件数量”相关，最稳但最慢；大树可能需要数分钟到十几分钟。 |
| 失败行为 | `zfs diff` 不可用、snapshot 缺失、baseline dataset 不匹配、检测到不安全嵌套挂载、diff 执行失败等情况会自动回退全树扫描，不会因为开着该选项而跳过必要对账。 | 没有 `zfs diff` 相关回退路径，因为一开始就走全树/事件路径。 |

简单说：`zfs_diff=true` 让“需要对账时如何找差异”优先相信 ZFS baseline diff；`zfs_diff=false` 强制 Full/Compare/fallback reconcile 重新看完整两棵树。日常调度到点的 Incremental 仍然是事件日志增量；不开 zfs diff 不等于每天全量，只是在必须对账时不用 ZFS 快路径。

建议使用方式：

- 平时保持开启：大 ZFS 树的 Full / Compare / fallback reconcile 会快很多。
- 怀疑 baseline 不可信、目标端被大量手工改过、刚经历磁盘/挂载异常时，临时关闭对应目标的 `sync.zfs_diff`，跑一次 Full 或 Compare，用完整双树扫描重新确认。
- 对非 ZFS 路径或跨机器非本机 ZFS 目标，开启也不会强行使用；前提不满足会自动回退。
- 如果只是想让每天 10 点的 scheduled sync 继续按事件路径增量跑，不需要开启 zfs diff；它只影响事件路径不可用后的 reconcile 快路径，以及手动 Full/Compare。

**跨机传输**：源/目标清单、批量 mkdir、批量 remove、文件推送都通过 peer HTTP transfer API 执行。本机 ZFS 源先创建 snapshot 只读视图供读取；当 Full / Compare / fallback reconcile 满足 baseline 前提时，可用 `zfs diff` 把对账范围裁剪到变化路径。传输细节见[性能优化点](#性能优化点)。
...
- **协作式取消**：所有目录遍历（每目录 + 每条目）、并行传输 worker（每文件）、分块大文件发送（每 16MiB）都有取消检查点，几十万条目的 HDD 扫描也能秒级停止。入口：`POST /api/cancel-activity`、UI 停止按钮、桌面 command。按 `source_id|destination_id` 精确定位，绝不误杀其他 pass；`propagate = true` 时转发到所有已知机器。取消后 cycle 的手动标记清除、in-flight target 撤销（verified 基准保留），下次 schedule/事件/手动同步重新拿到 target。
...
ssh_port = 10022
os = "linux"
install_dir = "/opt/usr/local/auto_sync"
```
...
Web 服务默认监听配置里的 `port`（通常 `18765`）。Web UI、Flutter 桌面端和脚本调用都走同一套 API。
