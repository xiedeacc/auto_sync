...
if [ -d /root/.vim/bundle/YouCompleteMe ]; then
    ...
    if [ -n "$ycm_commit" ] && [ "$(cat /root/.vim/bundle/YouCompleteMe/.auto_sync_installed 2>/dev/null || true)" != "$ycm_commit" ]; then
        ycm_path="$JAVA_HOME/bin:/usr/local/go/go1.25.1/bin:/root/src/go/bin:/root/.cargo/bin:/root/.nvm/versions/node/v24.18.0/bin:$PATH"
...
chmod -R a+rX /root/.nvm/versions/node/v24.18.0 /root/src/software/tools/uv-python 2>/dev/null || true
normalize_deploy_permissions
