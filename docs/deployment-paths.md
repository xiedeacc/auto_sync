...
| 组件 | AWS | dev | NAS | Windows | OpenWrt | test / 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| auto_sync | 不作为常规 auto_sync 部署目标。 | `/usr/local/auto_sync`<br>部署并运行；源码从 `/root/src/rust/auto_sync` 执行 `scripts/deploy_local.sh --install-dir /usr/local/auto_sync`；运行目录只保留部署产物和数据。 | `/opt/usr/local/auto_sync`<br>部署并运行；源码从 `/opt/src/rust/auto_sync` 执行 `scripts/deploy_nas.sh`；`/opt/usr/local/auto_sync` 只保留部署产物和数据。 | 部署并运行，使用 `scripts/deploy_windows.ps1`。 | 不作为常规 auto_sync 部署目标。 | - |
| rblog | 仍由 AWS collector 管理并启动 `rblog`、`rblog-backup.timer`，路径为 `/usr/local/blog`。 | 不由 collector 管理<br>不再采集、部署、启动或校验 rblog；现有文件如需保留/删除，用一次性维护命令处理。 | 不由 collector 管理<br>不再采集、部署、启动或校验 rblog；现有文件如需保留/删除，用一次性维护命令处理。 | 不运行。 | 不运行。 | test 不再采集、部署、启动或校验 rblog。 |
| shadowsocks | enable/start `shadowsocks-rust`。 | `/usr/local/shadowsocks`<br>保留文件但 disable/stop `shadowsocks` 和 `shadowsocks-rust`。 | `/opt/usr/local/shadowsocks`<br>保留文件但 disable/stop `shadowsocks` 和 `shadowsocks-rust`。 | 不运行。 | enable/start 非 rust 版 `shadowsocks`，并 disable/stop `shadowsocks-rust`。 | - |
| tbox | enable/start `tbox_server.service`。 | 不由 collector 管理<br>不再采集、部署、启动、停止或校验 `tbox_client` / `tbox-logrotate.service` / `tbox-logrotate.timer`；现有文件如需处理，用一次性维护命令。 | `/opt/usr/local/tbox`<br>enable/start `tbox_client.service` 和 `tbox-logrotate.timer`。 | 不运行。 | 不运行。 | - |
| waiwei | disable/stop `waiwei-web`、`waiwei-puller`。 | `/usr/local/waiwei`<br>disable/stop `waiwei-web`、`waiwei-puller`。 | `/opt/usr/local/waiwei`<br>disable/stop `waiwei-web`、`waiwei-puller`。 | 不运行。 | 不运行。 | 只有 `waiwei-web` 和 `waiwei-puller`，不存在单独的 `waiwei` systemd 服务；文件仅用于采集、备份或保留历史配置。 |
| xray | disable/stop 独立 `xray` 服务或进程。 | `/usr/local/xray`<br>disable/stop 独立 `xray` 服务或进程。 | `/opt/usr/local/xray`<br>disable/stop 独立 `xray` 服务或进程。 | 不运行。 | 不单独列出；`xray-plugin` 属于 shadowsocks 链路，不作为独立 xray 服务展示。 | - |
| immich | 不运行。 | 不由 collector 管理<br>不再采集、部署、启动、停止或校验 `immich`/`immich-ml`。 | 不由 collector 管理<br>collector 不再采集、部署、启动或校验 `immich`/`immich-ml`，部署脚本只负责 disable/stop 相关服务且不删除现有文件。 | 不运行。 | 不运行。 | - |
| domus | 不由这套 collector 部署脚本管理。 | 不由 collector 管理<br>不由这套 collector 部署脚本管理。 | `/opt/usr/local/domus`<br>采集 `/opt/usr/local/domus`、`domus.service`、`domus-backup.service/timer`；部署时 enable/start `domus` 和 `domus-backup.timer`。 | 不由这套 collector 部署脚本管理。 | 不由这套 collector 部署脚本管理。 | `/opt/usr/local/domus/.backup-worktree` 指向 `git@github.com:xiedeacc/domus_data.git`，周期性备份 `bin`、`conf` 和 `data` 中的小状态文件；`data/upload`、`data/backups`、logs、`.backup-worktree` 不进 collector/share/git 备份。 |
| rgit | 不由这套 nas/dev collector 脚本管理。 | 不由 collector 管理<br>不采集、不部署、不启动 rgit。 | `/opt/usr/local/rgit`<br>采集 `/opt/usr/local/rgit`、`rgit.service`、`rgit-backup.service/timer`、`rgit-ocsp.service/timer` 和 `rgit-backup.service.d`；部署时 enable/start `rgit`、`rgit-backup.timer`、`rgit-ocsp.timer`。 | 不运行。 | 不运行。 | - |
| gitlab | 不由 dev/nas collector 管理。 | 不由 collector 管理<br>collector 部署脚本不再安装、配置、启动或校验 GitLab，也不采集 `/etc/gitlab`。 | 不由 collector 管理<br>collector 部署脚本不再安装、配置、启动或校验 GitLab，也不采集 `/etc/gitlab`。 | 不由 dev/nas collector 管理。 | 不由 dev/nas collector 管理。 | 如需 GitLab 迁移或维护，用一次性命令或专门脚本处理。 |
| 数据库服务 | 不由 dev/nas collector 管理。 | collector 部署时可运行<br>仍按当前脚本运行这些服务。 | 保留配置/数据但 disable/stop `mysql`、`postgresql`、`redis-server`<br>不再运行 MySQL/PostgreSQL/Redis；部署脚本只保留配置和数据，不启动、不 enable、不作为 required service 校验。 | 不由 dev/nas collector 管理。 | 不由 dev/nas collector 管理。 | - |
| 数据库 dump/恢复 | 不由 dev/nas collector 管理。 | 不由 collector 管理<br>不再自动生成 MySQL/PostgreSQL dump，不再采集数据库 dump 脚本，不再部署时恢复 dump，也不再写入数据库备份 crontab。 | 不由 collector 管理<br>不再自动生成 MySQL/PostgreSQL dump，不再采集数据库 dump 脚本，不再部署时恢复 dump，也不再写入数据库备份 crontab。 | 不由 dev/nas collector 管理。 | 不由 dev/nas collector 管理。 | 现有数据库文件、备份文件或脚本如需保留/删除，用一次性维护命令处理。 |

### 其它路径与工具链

| 组件 | AWS | dev | NAS | Windows | OpenWrt | test / 备注 |
| --- | --- | --- | --- | --- | --- | --- |
| auto_sync 源码 | 不作为常规 auto_sync 源码主机。 | `/root/src/rust/auto_sync`<br>唯一 dev 源码 checkout；不要保留 `/root/src/auto_sync`。 | `/opt/src/rust/auto_sync`<br>唯一 nas 源码 checkout；不要把 git 仓库放在 `/opt/usr/local/auto_sync`。 | 源码位于 Windows 工作区。 | 不放源码。 | - |
| go sdk | 不由 collector 部署脚本管理。 | `/usr/local/go/go1.25.1`<br>使用正常的 `/usr/local/go`。 | `/opt/usr/local/go/go1.25.1`<br>通过 `/etc/profile.d/opt-usr-local-path.sh` 把 go 加入 `PATH`。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| gopath | 不由 collector 部署脚本管理。 | `/root/src/go`<br>使用 `/root/src/go`，不要重新创建 `/root/go`。 | `/root/src/go`，通过 `/opt/user/root/src/go` symlink 指向<br>使用 `/root/src/go`，实际通过 symlink 指到 `/opt/user/root/src/go`；不要重新创建 `/root/go`。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| bin | 不由 collector 部署脚本管理。 | `/usr/local/bin`<br>使用 `/usr/local/bin`。 | `/opt/usr/local/bin`<br>使用 `/opt/usr/local/bin`；不要把 `/usr/local/bin` 重新做成 bind mount。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| buildifier | 不由 collector 部署脚本管理。 | `/usr/local/bin/buildifier`<br>由 collector 部署脚本安装到 `/usr/local/bin/buildifier`。 | `/opt/usr/local/bin/buildifier`<br>由 collector 部署脚本安装到 `/opt/usr/local/bin/buildifier`。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| 静态站点根目录 | 不由 nas/dev collector 默认路径管理。 | 不由 collector 默认采集<br>`/opt/www` 不在 `conf/collector.toml` 默认 collect 列表中，部署脚本不自动从远端采集、创建 coverage 或补路径。 | 不由 collector 默认采集<br>`/opt/www` 不在 `conf/collector.toml` 默认 collect 列表中，部署脚本不自动从远端采集或补路径。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| flutter sdk | 不安装。 | `/root/src/software/flutter`<br>`/root/src/software/flutter`。 | `/opt/src/software/flutter`<br>`/opt/src/software/flutter`，wrapper 设置 `FLUTTER_ROOT`。 | 使用 Windows 本机 flutter/构建环境，不使用该 Linux 路径。 | 不安装。 | - |
| nvm root | 不由 collector 部署脚本管理。 | `/root/src/software/tools/nvm`<br>`/root/src/software/tools/nvm`，作为 `NVM_DIR`。 | `/opt/src/software/tools/nvm`<br>`/opt/src/software/tools/nvm`，作为 `NVM_DIR`；不要重建 tiger 用户 nvm/npm 状态。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| node.js | 不由 collector 部署脚本管理。 | `/root/src/software/tools/nvm/versions/node/v24.18.0`<br>由 nvm 安装，路径在 `/root/src/software/tools/nvm` 下；可回退到最新 Node 24。 | `/opt/src/software/tools/nvm/versions/node/v24.18.0`<br>由 nvm 安装，路径在 `/opt/src/software/tools/nvm` 下；可回退到最新 Node 24。 | 使用 Windows 本机 Node，不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| npm | 不由 collector 部署脚本管理。 | nvm root 下的 node.js `bin/npm`<br>使用 nvm root 下的 `npm`，registry 由部署脚本配置。 | nvm root 下的 node.js `bin/npm`<br>使用 nvm root 下的 `npm`，registry 由部署脚本配置。 | 使用 Windows 本机 npm。 | 不使用该 Linux 路径。 | - |
| pnpm | 不由 collector 部署脚本管理。 | nvm root 下由 node.js/Corepack 管理的二进制<br>Node 设置完成后安装或激活。 | nvm root 下由 node.js/Corepack 管理的二进制<br>Node 设置完成后安装或激活。 | 使用 Windows 本机 pnpm/Corepack。 | 不使用该 Linux 路径。 | - |
| python | 不由这套 nas/dev collector 脚本管理。 | apt 的系统 python，通常是 `/usr/bin/python3`<br>使用 apt 系统 python；项目 virtualenv 留在各自项目目录内。 | apt 的系统 python，通常是 `/usr/bin/python3`<br>使用 apt 系统 python；项目 virtualenv 留在各自项目目录内。 | 使用 Windows 本机 python。 | 不使用该 Linux 路径。 | - |
| pip | 不由这套 nas/dev collector 脚本管理。 | `/etc/pip.conf`<br>collector 部署脚本管理 `/etc/pip.conf`。 | `/etc/pip.conf`<br>collector 部署脚本管理 `/etc/pip.conf`。 | 不使用该 Linux 配置文件。 | 不使用该 Linux 配置文件。 | - |
| uv | 不由这套 nas/dev collector 脚本管理。 | `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量<br>通过 `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量配置。 | `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量<br>通过 `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量配置。 | 不使用该 Linux 配置文件。 | 不使用该 Linux 配置文件。 | - |
| rust | 不由这套 nas/dev collector 脚本管理。 | `/root/.cargo`、`/root/.rustup` 真实本地路径<br>`/root/.cargo`、`/root/.rustup` 是真实本地路径；Cargo registry 镜像配置位于 `/root/.cargo/config.toml`。 | `/root/.cargo`、`/root/.rustup` symlink 到 `/opt/user/root`<br>`/root/.cargo`、`/root/.rustup` symlink 到 `/opt/user/root`，避免占用根分区。 | 使用 Windows 本机 rust 工具链。 | 不使用该 Linux 路径。 | - |
| java | 不由这套 nas/dev collector 脚本管理。 | apt openjdk，路径 `/usr/lib/jvm/java-21-openjdk-amd64`<br>使用 apt openjdk，`JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64`。 | apt openjdk，路径 `/usr/lib/jvm/java-21-openjdk-amd64`<br>使用 apt openjdk，`JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64`。 | 使用 Windows 本机 JDK；不要从该表推导 Windows 路径。 | 不使用该 Linux 路径。 | - |
| pgvector | 不由这套 nas/dev collector 脚本管理。 | `/root/src/software`<br>使用 `/root/src/software`。 | `/opt/src/software`<br>使用 `/opt/src/software`，避免占用根分区。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| openwrt toolchain | 不由这套 nas/dev collector 脚本管理。 | `/root/src/software/openwrt`<br>`scripts/deploy_openwrt.sh` 优先从 `/root/src/software/openwrt` 自动发现 `toolchain-aarch64_*_musl`。 | 不使用<br>不作为 OpenWrt 构建主机。 | 不使用该 Linux 路径。 | 只接收构建产物。 | - |
| root home | 不使用 nas 的 `/opt/user/root` 布局。 | 无；`/root` 是真实路径<br>`/root` 是真实路径，不创建 `/opt/user/root`。 | `/opt/user/root`<br>`/root` 下除 `/root/.ssh` 外的所有子项 symlink 到 `/opt/user/root`；`/root/.ssh` 保持真实目录以保证 SSH 登录安全。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
| tiger home | 不使用 nas 的 `/opt/user/tiger` 布局。 | 无；`/home/tiger` 是真实路径<br>`/home/tiger` 是真实路径，不创建 `/opt/user/tiger`。 | `/opt/user/tiger`<br>选定的 `/home/tiger` dotfile symlink 到 `/opt/user/tiger`。 | 不使用该 Linux 路径。 | 不使用该 Linux 路径。 | - |
