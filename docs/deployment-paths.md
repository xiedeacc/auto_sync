...
| halo_data | `/opt/src/software/halo_data` | 不使用 | • nas: Halo2 自包含资源包，保存 `bin/`、`runtime/.halo2/`、`systemd/halo2.service`、`backups/mysql/` 和 `deploy_halo.sh`。<br>• 默认部署到 `/opt/usr/local/halo`，也可执行 `deploy_halo.sh --install-dir /usr/local/halo` 部署到旧路径。<br>• dev/windows/openwrt/aws: 不使用该资源包。 |
