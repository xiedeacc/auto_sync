...
| rblog | 仍由 AWS collector 管理并启动 `rblog`、`rblog-backup.timer`，路径为 `/usr/local/blog`。 | 不由 collector 管理<br>不再采集、部署、启动或校验 rblog；现有文件如需保留/删除，用一次性维护命令处理。 | 不由 collector 管理<br>不再采集、部署、启动或校验 rblog；现有文件如需保留/删除，用一次性维护命令处理。 | 不运行。 |
...
| waiwei | disable/stop `waiwei-web`、`waiwei-puller`。 | `/usr/local/waiwei`<br>disable/stop `waiwei-web`、`waiwei-puller`。 | `/opt/usr/local/waiwei`<br>disable/stop `waiwei-web`、`waiwei-puller`。 | 不运行。 |
...
| domus | 不由这套 collector 部署脚本管理。 | 不由 collector 管理<br>不由这套 collector 部署脚本管理。 | `/opt/usr/local/domus`<br>采集 `/opt/usr/local/domus`、`domus.service`、`domus-backup.service/timer`；部署时 enable/start `domus` 和 `domus-backup.timer`。 | 不由这套 collector 部署脚本管理。 |
...
| gitlab | 不由 dev/nas collector 管理。 | 不由 collector 管理<br>collector 部署脚本不再安装、配置、启动或校验 GitLab，也不采集 `/etc/gitlab`。 | 不由 collector 管理<br>collector 部署脚本不再安装、配置、启动或校验 GitLab，也不采集 `/etc/gitlab`。 | 不由 dev/nas collector 管理。 |
