...
| halo home | `/root/.halo2` symlink 到 `/opt/user/root/.halo2` | `/root/.halo2` 真实本地路径 | • aws: 不使用。<br>• dev: `/root/.halo2` 是真实本地路径，`halo2.service` 的 `WorkingDirectory` 指向这里。<br>• nas: `/root/.halo2` symlink 到 `/opt/user/root/.halo2`，`halo2.service` 的 `WorkingDirectory` 指向 `/root/.halo2`，实际落盘到 `/opt/user/root/.halo2`。<br>• windows: 不使用该 Linux 路径。<br>• openwrt: 不使用。<br>• 备注: 旧 `.halo` 是 halo 1.x 遗留路径，不应再创建。 |
| halo_data | `/opt/src/software/halo_data` | 不使用 | • nas: Halo2 自包含资源包，保存 `bin/`、`runtime/.halo2/`、`systemd/halo2.service`、`backups/mysql/` 和 `deploy_halo.sh`。<br>• 默认部署到 `/opt/usr/local/halo`，也可执行 `deploy_halo.sh --install-dir /usr/local/halo` 部署到旧路径。<br>• 每次默认重建并恢复 `halodb`，同时初始化 `tiger@localhost` 和 `tiger@127.0.0.1`。只有明确传 `--skip-db-restore` 才不触碰数据库。<br>• dev/windows/openwrt/aws: 不使用该资源包。 |
...
| immich | `/opt/immich` | `/usr/local/immich` | • aws: 不运行。<br>• dev: 启动 `immich`、`immich-ml`，路径为 `/usr/local/immich`，以 root 运行。<br>• nas: 启动 `immich`、`immich-ml`，路径为 `/opt/immich`，以 root 运行。<br>• windows: 不运行。<br>• openwrt: 不运行。 |
...
nas 不允许把 `/opt/usr/local` bind-mount 回 `/usr/local`。旧路径如 `/usr/local/blog`、`/usr/local/go`、`/usr/local/halo`、`/usr/local/tbox`、`/usr/local/waiwei`、`/usr/local/xray`、`/usr/local/bin` 应通过一次性命令或临时迁移脚本处理；长期 collector 部署脚本只维护当前 `/opt/usr/local/*` 目标布局。
...
systemd unit 文件应使用上表中的绝对真实路径。`scripts/deploy_local.sh` 会根据选定安装目录直接渲染 `auto_sync.service`；`scripts/deploy_nas.sh` 提供 nas 路径。`halo2.service` 虽然以 root 用户运行，但 `WorkingDirectory` 必须指向 `/root/.halo2`，避免在 `/root` 顶层生成 `search-index`、`themes`、`uploads` 等运行目录。
