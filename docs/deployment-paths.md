...
| --- | --- | --- | --- |
| auto_sync | `/opt/usr/local/auto_sync` | `/usr/local/auto_sync` | NAS 使用 `scripts/deploy_nas.sh`；dev 使用 `scripts/deploy_local.sh --install-dir /usr/local/auto_sync`。 |
| rblog | `/opt/usr/local/blog` | `/usr/local/blog` | service unit 必须使用对应主机的路径。NAS 的 `.backup-worktree` 位于 `/opt/usr/local/blog` 下。 |
...
| Halo 安装目录 | `/opt/usr/local/halo` | `/usr/local/halo` | 两台机器上都以 root 运行。 |
| Halo 运行时 home | `/root/.halo2` symlink 到 `/opt/user/root/.halo2` | `/root/.halo2` 真实本地路径 | Halo 2 使用 `.halo2`；旧的 `.halo` 是 Halo 1.x 遗留路径，不应再创建。NAS 将 root home 状态放到根分区之外。 |
| shadowsocks | `/opt/usr/local/shadowsocks` | `/usr/local/shadowsocks` | 支持采集配置、数据和日志目录；当 xray 占用端口时，服务启动可以被禁用。 |
| TBox | `/opt/usr/local/tbox` | `/usr/local/tbox` | `tbox_client.service` 和 logrotate 路径必须指向对应主机路径。 |
| Waiwei | `/opt/usr/local/waiwei` | `/usr/local/waiwei` | `waiwei-web` 和 `waiwei-puller` unit 从这里运行。 |
| Xray | `/opt/usr/local/xray` | `/usr/local/xray` | `xray.service` 使用这里的二进制和数据。 |
| Immich 运行目录 | `/opt/immich` | `/usr/local/immich` | 原生部署，不使用 Docker。 |
| Immich 源码 checkout | `/opt/src/software/immich` | `/root/src/software/immich` | 使用 `deploy` 分支。 |
| GitLab 仓库数据 | 导入的 NAS ZFS 池上的 `/zfs/gitlab_data` | 普通目录 `/zfs/gitlab_data` | collector 部署脚本将 GitLab 仓库和 LFS 存储配置到这里。test 也使用同一路径，但只是普通目录。 |
...
| 静态站点根目录 | `/opt/www/gitlab_cleaner`、`/opt/www/unlock-music` | `/opt/www/gitlab_cleaner`、`/opt/www/unlock-music`、`/opt/www/coverage` | collector 部署脚本会补充这些平台默认路径，即使 collector 列表还没写全。 |
...
| 入口 | 默认安装目录 | NAS 覆盖值 |
| --- | --- | --- |
| `scripts/deploy_local.sh` | `/usr/local/auto_sync` | `scripts/deploy_nas.sh` 设置为 `/opt/usr/local/auto_sync`。 |
...
| `/etc/hosts` | NAS/dev/test | collector 部署脚本 | 确保本地服务域名指向预期的 NAS/dev 地址，并移除重复的陈旧项。 |
