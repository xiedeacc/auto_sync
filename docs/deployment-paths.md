...
| --- | --- | --- | --- | --- |
| shadowsocks | • 部署目录: `/usr/local/shadowsocks`<br>• 状态: 停止/禁用 | • 部署目录: `/opt/usr/local/shadowsocks`<br>• 状态: 停止/禁用 | • 服务: `shadowsocks-rust`<br>• 状态: 运行 | • 服务: `shadowsocks`<br>• 状态: 运行<br>• 备注: `xray-plugin` 属于 shadowsocks 链路 |
| xray | • 部署目录: `/usr/local/xray`<br>• 状态: 停止/禁用 | • 部署目录: `/opt/usr/local/xray`<br>• 状态: 停止/禁用 | • 服务: `xray`<br>• 状态: 停止/禁用 | × |
| tbox | × | • 部署目录: `/opt/usr/local/tbox`<br>• 状态: `tbox_client.service` 运行<br>• 日志轮转: `tbox-logrotate.timer` 定时触发 `tbox-logrotate.service` | • 服务: `tbox_server.service`<br>• 状态: 运行 | × |
| rblog | × | × | • 部署目录: `/usr/local/blog`<br>• 状态: 运行<br>• 备份: `rblog-backup.timer` 定时触发 `rblog-backup.service` | × |
| waiwei | • 部署目录: `/usr/local/waiwei`<br>• 状态: 停止/禁用<br>• 备注: 仅保留文件 | • 部署目录: `/opt/usr/local/waiwei`<br>• 状态: 停止/禁用<br>• 备注: 仅保留文件 | • 服务: `waiwei-web`、`waiwei-puller`<br>• 状态: 停止/禁用 | × |
| auto_sync | • 部署目录: `/usr/local/auto_sync`<br>• 状态: 运行 | • 部署目录: `/opt/usr/local/auto_sync`<br>• 状态: 运行 | × | × |
| domus | × | • 部署目录: `/opt/usr/local/domus`<br>• 状态: 运行<br>• 备份: `domus-backup.timer` 定时备份 `bin`、`conf`、`data` 小状态文件 | × | × |
| rgit | × | • 部署目录: `/opt/usr/local/rgit`<br>• 状态: 运行<br>• 备份: `rgit-backup.timer`<br>• OCSP: `rgit-ocsp.timer` | × | × |
| mysql | • 状态: 运行 | • 数据: 保留<br>• 状态: 停止/禁用 | × | × |
| postgres | • 状态: 运行 | • 数据: 保留<br>• 状态: 停止/禁用 | × | × |
| redis | • 状态: 运行 | • 数据: 保留<br>• 状态: 停止/禁用 | × | × |
