...
| 主机 | 文件 | 作用 |
| --- | --- | --- |
| nas/dev | `/etc/profile` | apt openjdk 的 `JAVA_HOME` 和 Java `PATH`。下载加速配置不要长期写入 profile 环境变量。 |
...
| `/etc/profile` | nas/dev | collector 文件；collector 部署脚本维护 `# BEGIN auto_sync java environment` 托管块 | 持久化 apt openjdk 的 `JAVA_HOME` 和 Java `PATH`。 |
...
| 变量 | 值 | 作用域 | 管理方 | 说明 |
| --- | --- | --- | --- | --- |
| `GOPROXY` | `https://goproxy.cn,direct` | collector 部署过程中进程内生效 | collector 部署脚本 | Go module 镜像；长期配置写入 root `.config/go/env`。 |
| `NVM_NODEJS_ORG_MIRROR` | `https://npmmirror.com/mirrors/node` | collector 部署过程中进程内生效 | collector 部署脚本 | nvm 的 Node 二进制下载镜像；只用于当前安装过程。 |
| `npm_config_registry` | `https://registry.npmmirror.com` | collector 部署过程中进程内生效 | collector 部署脚本 | npm registry 镜像；长期配置写入 root `.npmrc`。 |
| `COREPACK_NPM_REGISTRY` | `https://registry.npmmirror.com` | collector 部署过程中进程内生效 | collector 部署脚本 | Corepack package registry；只用于当前安装过程。 |
| `RUSTUP_DIST_SERVER` | `https://rsproxy.cn` | collector 部署过程中进程内生效 | collector 部署脚本、`scripts/deploy_local.sh` | rustup distribution 镜像；只用于当前安装过程。 |
| `RUSTUP_UPDATE_ROOT` | `https://rsproxy.cn/rustup` | collector 部署过程中进程内生效 | collector 部署脚本、`scripts/deploy_local.sh` | rustup metadata 镜像；只用于当前安装过程。 |
