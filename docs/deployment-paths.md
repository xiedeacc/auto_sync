...
| 组件 | dev | NAS | AWS | OpenWrt |
| --- | --- | --- | --- | --- |
| shadowsocks | • 部署目录: `/usr/local/shadowsocks`<br>• 状态: disable/stop | • 部署目录: `/opt/usr/local/shadowsocks`<br>• 状态: disable/stop | • 部署目录: `/usr/local/shadowsocks`<br>• 状态: enable/start | • 部署目录: `/usr/local/shadowsocks`<br>• 状态: enable/start |
| xray | • 部署目录: `/usr/local/xray`<br>• 状态: disable/stop | • 部署目录: `/opt/usr/local/xray`<br>• 状态: disable/stop | • 部署目录: `/usr/local/xray`<br>• 状态: disable/stop | × |
| tbox | × | • 部署目录: `/opt/usr/local/tbox`<br>• 状态: client enable/start | • 部署目录: `/usr/local/tbox`<br>• 状态: server enable/start | × |
| rblog | × | × | • 部署目录: `/usr/local/blog`<br>• 状态: enable/start<br>• 备份: `rblog-backup.timer` -> `rblog-backup.service` | × |
| waiwei | • 部署目录: `/usr/local/waiwei`<br>• 状态: waiwei-puller disable/stop，waiwei-web disable/stop | • 部署目录: `/opt/usr/local/waiwei`<br>• 状态: waiwei-puller disable/stop，waiwei-web disable/stop | • 部署目录: `/usr/local/waiwei`<br>• 状态: waiwei-puller disable/stop，waiwei-web disable/stop | × |
| auto_sync | • 部署目录: `/usr/local/auto_sync`<br>• 状态: enable/start | • 部署目录: `/opt/usr/local/auto_sync`<br>• 状态: enable/start | × | × |
| domus | × | • 部署目录: `/opt/usr/local/domus`<br>• 状态: enable/start<br>• 备份: `domus-backup.timer` -> `domus-backup.service` | × | × |
| rgit | × | • 部署目录: `/opt/usr/local/rgit`<br>• 状态: enable/start<br>• 备份: `rgit-backup.timer` -> `rgit-backup.service` | × | × |
| mysql | • 部署目录: `/etc/mysql`<br>• 状态: enable/start | • 部署目录: `/etc/mysql`<br>• 状态: disable/stop | × | × |
| postgres | • 部署目录: `/etc/postgresql`<br>• 状态: enable/start | • 部署目录: `/etc/postgresql`<br>• 状态: disable/stop | × | × |
| redis | • 部署目录: 系统默认<br>• 状态: enable/start | • 部署目录: 系统默认<br>• 状态: disable/stop | × | × |

### 其它路径与工具链

| 组件 | dev | NAS | AWS | OpenWrt |
| --- | --- | --- | --- | --- |
| go sdk | • 路径: `/usr/local/go/go1.25.1`<br>• 状态: 使用 | • 路径: `/opt/usr/local/go/go1.25.1`<br>• 状态: 使用<br>• PATH: `/etc/profile.d/opt-usr-local-path.sh` | × | × |
| gopath | • 路径: `/root/src/go`<br>• 状态: 使用 | • 路径: `/root/src/go` -> `/opt/user/root/src/go`<br>• 状态: 使用 | × | × |
| bin | • 路径: `/usr/local/bin`<br>• 状态: 使用 | • 路径: `/opt/usr/local/bin`<br>• 状态: 使用 | × | × |
| buildifier | • 路径: `/usr/local/bin/buildifier`<br>• 状态: 安装 | • 路径: `/opt/usr/local/bin/buildifier`<br>• 状态: 安装 | × | × |
| 静态站点根目录 | × | × | × | × |
| flutter sdk | • 路径: `/root/src/software/flutter`<br>• 状态: 使用 | • 路径: `/opt/src/software/flutter`<br>• 状态: 使用<br>• 环境: `FLUTTER_ROOT` | × | × |
| nvm root | • 路径: `/root/src/software/tools/nvm`<br>• 状态: `NVM_DIR` | • 路径: `/opt/src/software/tools/nvm`<br>• 状态: `NVM_DIR` | × | × |
| node.js | • 路径: `/root/src/software/tools/nvm/versions/node/v24.18.0`<br>• 状态: 使用 | • 路径: `/opt/src/software/tools/nvm/versions/node/v24.18.0`<br>• 状态: 使用 | × | × |
| npm | • 路径: nvm node `bin/npm`<br>• 状态: 使用 | • 路径: nvm node `bin/npm`<br>• 状态: 使用 | × | × |
| pnpm | • 路径: nvm/Corepack 管理<br>• 状态: 使用 | • 路径: nvm/Corepack 管理<br>• 状态: 使用 | × | × |
| python | • 路径: `/usr/bin/python3`<br>• 状态: 使用 | • 路径: `/usr/bin/python3`<br>• 状态: 使用 | × | × |
| pip | • 路径: `/etc/pip.conf`<br>• 状态: 管理 | • 路径: `/etc/pip.conf`<br>• 状态: 管理 | × | × |
| uv | • 路径: `/etc/profile.d/auto-sync-domestic-mirrors.sh`<br>• 状态: 管理 | • 路径: `/etc/profile.d/auto-sync-domestic-mirrors.sh`<br>• 状态: 管理 | × | × |
| rust | • 路径: `/root/.cargo`、`/root/.rustup`<br>• 状态: 真实路径 | • 路径: `/root/.cargo`、`/root/.rustup` -> `/opt/user/root`<br>• 状态: symlink | × | × |
| java | • 路径: `/usr/lib/jvm/java-21-openjdk-amd64`<br>• 状态: 使用 | • 路径: `/usr/lib/jvm/java-21-openjdk-amd64`<br>• 状态: 使用 | × | × |
| pgvector | • 路径: `/root/src/software`<br>• 状态: 使用 | • 路径: `/opt/src/software`<br>• 状态: 使用 | × | × |
| openwrt toolchain | • 路径: `/root/src/software/openwrt`<br>• 状态: 构建使用 | × | × | • 状态: 接收构建产物 |
| root home | • 路径: `/root`<br>• 状态: 真实路径 | • 路径: `/opt/user/root`<br>• 状态: `/root` 子项 symlink，`/root/.ssh` 除外 | × | × |
| tiger home | • 路径: `/home/tiger`<br>• 状态: 真实路径 | • 路径: `/opt/user/tiger`<br>• 状态: `/home/tiger` 选定 dotfile symlink | × | × |
