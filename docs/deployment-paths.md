...
| tbox | `/opt/usr/local/tbox` | 不由 collector 管理 | • aws: enable/start `tbox_server.service`。<br>• dev: 不再采集、部署、启动、停止或校验 `tbox_client` / `tbox-logrotate.service` / `tbox-logrotate.timer`；现有文件如需处理，用一次性维护命令。<br>• nas: enable/start `tbox_client.service` 和 `tbox-logrotate.timer`。<br>• windows: 不运行。<br>• openwrt: 不运行。 |
...
| immich | 不由 collector 管理 | 不由 collector 管理 | • dev: 不再采集、部署、启动、停止或校验 `immich`/`immich-ml`。<br>• nas: collector 不再采集、部署、启动或校验 `immich`/`immich-ml`，部署脚本只负责 disable/stop 相关服务且不删除现有文件。<br>• aws/windows/openwrt: 不运行。 |
