...
| rblog | × | × | `/usr/local/blog`<br>由 AWS collector 管理并启动 `rblog`、`rblog-backup.timer`；`rblog-backup.timer` 定时触发 `rblog-backup.service`，`rblog-backup.service` 执行一次实际备份。 | × |
| shadowsocks | `/usr/local/shadowsocks`<br>保留文件但 disable/stop `shadowsocks` 和 `shadowsocks-rust`。 | `/opt/usr/local/shadowsocks`<br>保留文件但 disable/stop `shadowsocks` 和 `shadowsocks-rust`。 | enable/start `shadowsocks-rust`。 | enable/start 非 rust 版 `shadowsocks`，并 disable/stop `shadowsocks-rust`；`xray-plugin` 属于 shadowsocks 链路。 |
| tbox | × | `/opt/usr/local/tbox`<br>enable/start `tbox_client.service` 和 `tbox-logrotate.timer`；`tbox-logrotate.timer` 定时触发 `tbox-logrotate.service`，`tbox-logrotate.service` 执行一次日志轮转。 | enable/start `tbox_server.service`。 | × |
| waiwei | `/usr/local/waiwei`<br>disable/stop `waiwei-web`、`waiwei-puller`；文件仅用于采集、备份或保留历史配置。 | `/opt/usr/local/waiwei`<br>disable/stop `waiwei-web`、`waiwei-puller`；文件仅用于采集、备份或保留历史配置。 | disable/stop `waiwei-web`、`waiwei-puller`；不存在单独的 `waiwei` systemd 服务。 | × |
| xray | `/usr/local/xray`<br>disable/stop 独立 `xray` 服务或进程。 | `/opt/usr/local/xray`<br>disable/stop 独立 `xray` 服务或进程。 | disable/stop 独立 `xray` 服务或进程。 | × |
