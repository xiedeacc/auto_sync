...
| Waiwei | `/opt/usr/local/waiwei` | `/usr/local/waiwei` | 保留文件采集和部署路径，但 `waiwei-web`、`waiwei-puller` 默认 disable/stop。 |
| Xray | `/opt/usr/local/xray` | `/usr/local/xray` | 保留文件采集和部署路径，但 `xray.service` 默认 disable/stop。 |
