...
| 主机 | 文件 | 作用 |
| --- | --- | --- |
| nas/dev/vmware | `/etc/environment` | 个人主机的全局工具链环境。sshd 必须保持 `UsePAM yes`，让 SSH login、interactive、non-interactive 会话都能看到。 |
...
| `/etc/ssh/sshd_config` | nas/dev/vmware | collector 部署脚本、主机级配置 | bootstrap 主机时强制项目 SSH 策略；必须保持 `UsePAM yes`，让 `/etc/environment` 对 Codex Desktop 的 root 非交互 SSH 检测生效。 |
...
个人主机把下列变量全部写入 `/etc/environment`，作为长期工具链环境的主配置。`/etc/environment` 不是 shell 脚本，`PATH`、`GOBIN` 等值必须使用完整绝对路径。sshd 必须保持 `UsePAM yes`，让 Codex Desktop 的 root 非交互 SSH 检测也能看到 `PATH` 和 `NVM_DIR`；不要用 `/usr/local/bin` symlink 或 `/root/.ssh/environment` 固化工具链路径。java 只使用 `JAVA_HOME`；不要设置全局 `CLASSPATH`/`CLASS_PATH`。collector 部署脚本会显式清理 root 和 tiger profile 中陈旧的 `CLASSPATH` 以及旧 `/usr/local/java` 启动项，所以每个 java 项目自行负责自己的 classpath。
...
| `PATH` | 主机真实 Node v24、Java、Go、`GOBIN`、Cargo、Flutter 和系统目录的绝对路径 | nas/dev/vmware 持久化 | `/etc/environment` | 让 Codex Desktop 的非交互 SSH 检测也能直接找到 `node`、`npm`、`codex`、`claude`。 |
