...
| 静态站点根目录 | `/opt/www/gitlab_cleaner`、`/opt/www/unlock-music` | `/opt/www/gitlab_cleaner`、`/opt/www/unlock-music` | • aws: 不由 nas/dev collector 默认路径管理。<br>• dev: 是否采集由 `conf/collector.toml` 的 collect 列表决定；部署脚本只消费本地 share 里已经采集到的目录，不再自动从远端采集、创建 coverage 或补路径。<br>• nas: 是否采集由 `conf/collector.toml` 的 collect 列表决定；部署脚本只消费本地 share 里已经采集到的目录，不再自动从远端采集或补路径。<br>• windows: 不使用该 Linux 路径。<br>• openwrt: 不使用该 Linux 路径。 |
