...
| bin 工具 | `/opt/usr/local/bin` | `/usr/local/bin` | • aws: 不由 collector 部署脚本管理。<br>• dev: 使用 `/usr/local/bin`。<br>• NAS: 使用 `/opt/usr/local/bin`；不要把 `/usr/local/bin` 重新做成 bind mount。<br>• Windows: 不使用该 Linux 路径。<br>• openwrt: 不使用该 Linux 路径。 |
...
| Halo 安装目录 | `/opt/usr/local/halo` | `/usr/local/halo` | • aws: 不运行。<br>• dev: 启动 `halo2.service`，以 root 运行。<br>• NAS: 启动 `halo2.service`，以 root 运行。<br>• Windows: 不运行。<br>• openwrt: 不运行。 |
| Halo 运行时 home | `/root/.halo2` symlink 到 `/opt/user/root/.halo2` | `/root/.halo2` 真实本地路径 | • aws: 不使用。<br>• dev: `/root/.halo2` 是真实本地路径。<br>• NAS: `/root/.halo2` symlink 到 `/opt/user/root/.halo2`，避免占用根分区。<br>• Windows: 不使用该 Linux 路径。<br>• openwrt: 不使用。<br>• 备注: 旧 `.halo` 是 Halo 1.x 遗留路径，不应再创建。 |
...
| Waiwei | `/opt/usr/local/waiwei` | `/usr/local/waiwei` | • aws: disable/stop `waiwei`、`waiwei-web`、`waiwei-puller`。<br>• dev: disable/stop `waiwei`、`waiwei-web`、`waiwei-puller`。<br>• NAS: disable/stop `waiwei`、`waiwei-web`、`waiwei-puller`。<br>• Windows: 不运行。<br>• openwrt: 不运行。<br>• 备注: 文件仅用于采集、备份或保留历史配置。 |
...
| Immich 运行目录 | `/opt/immich` | `/usr/local/immich` | • aws: 不运行。<br>• dev: 启动 `immich`、`immich-ml`，路径为 `/usr/local/immich`。<br>• NAS: 启动 `immich`、`immich-ml`，路径为 `/opt/immich`。<br>• Windows: 不运行。<br>• openwrt: 不运行。 |
| Immich 源码 checkout | `/opt/src/software/immich` | `/root/src/software/immich` | • aws: 不使用。<br>• dev: `/root/src/software/immich`，使用 `deploy` 分支。<br>• NAS: `/opt/src/software/immich`，使用 `deploy` 分支。<br>• Windows: 不使用该 Linux 路径。<br>• openwrt: 不使用。 |
| GitLab 仓库数据 | 导入的 NAS ZFS 池上的 `/zfs/gitlab_data` | 普通目录 `/zfs/gitlab_data` | • aws: 不运行 GitLab。<br>• dev: 启动 GitLab，`/zfs/gitlab_data` 是普通目录。<br>• NAS: 启动 GitLab，`/zfs/gitlab_data` 位于导入的 NAS ZFS 池。<br>• Windows: 不运行 GitLab。<br>• openwrt: 不运行 GitLab。 |
...
| pip 配置 | `/etc/pip.conf` | `/etc/pip.conf` | • aws: 不由这套 NAS/dev collector 脚本管理。<br>• dev: collector 部署脚本管理 `/etc/pip.conf`。<br>• NAS: collector 部署脚本管理 `/etc/pip.conf`。<br>• Windows: 不使用该 Linux 配置文件。<br>• openwrt: 不使用该 Linux 配置文件。 |
| uv 配置 | `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量 | `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量 | • aws: 不由这套 NAS/dev collector 脚本管理。<br>• dev: 通过 `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量配置。<br>• NAS: 通过 `/etc/profile.d/auto-sync-domestic-mirrors.sh` 中的环境变量配置。<br>• Windows: 不使用该 Linux 配置文件。<br>• openwrt: 不使用该 Linux 配置文件。 |
| Rust 工具链 | `/root/.cargo`、`/root/.rustup` symlink 到 `/opt/user/root` | `/root/.cargo`、`/root/.rustup` 真实本地路径 | • aws: 不由这套 NAS/dev collector 脚本管理。<br>• dev: `/root/.cargo`、`/root/.rustup` 是真实本地路径；Cargo registry 镜像配置位于 `/root/.cargo/config.toml`。<br>• NAS: `/root/.cargo`、`/root/.rustup` symlink 到 `/opt/user/root`，避免占用根分区。<br>• Windows: 使用 Windows 本机 Rust 工具链。<br>• openwrt: 不使用该 Linux 路径。 |
...
| pgvector 和源码工具 | `/opt/src/software` | `/root/src/software` | • aws: 不由这套 NAS/dev collector 脚本管理。<br>• dev: 使用 `/root/src/software`，包含 `aarch64-linux-musl-cross` 等源码工具。<br>• NAS: 使用 `/opt/src/software`，避免占用根分区。<br>• Windows: 不使用该 Linux 路径。<br>• openwrt: 不使用该 Linux 路径。 |
| root home 外置目录 | `/opt/user/root` | 无；`/root` 是真实路径 | • aws: 不使用 NAS 的 `/opt/user/root` 布局。<br>• dev: `/root` 是真实路径，不创建 `/opt/user/root`。<br>• NAS: `/root` 下除 `/root/.ssh` 外的所有子项 symlink 到 `/opt/user/root`；`/root/.ssh` 保持真实目录以保证 SSH 登录安全。<br>• Windows: 不使用该 Linux 路径。<br>• openwrt: 不使用该 Linux 路径。 |
| tiger home 外置目录 | `/opt/user/tiger` | 无；`/home/tiger` 是真实路径 | • aws: 不使用 NAS 的 `/opt/user/tiger` 布局。<br>• dev: `/home/tiger` 是真实路径，不创建 `/opt/user/tiger`。<br>• NAS: 选定的 `/home/tiger` dotfile symlink 到 `/opt/user/tiger`。<br>• Windows: 不使用该 Linux 路径。<br>• openwrt: 不使用该 Linux 路径。 |
