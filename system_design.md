...
auto_sync 是一个使用 Rust 实现的目录同步工具，支持图形界面配置多组源目录和目标目录。对 ZFS source，系统以 ZFS snapshot + `zfs diff` 作为强一致基础；fanotify 仅作为 realtime 策略下的近实时触发优化。系统需要把 cycle、snapshot、事件提示和 dst offset 持久化，且在进程重启、目标盘离线、fanotify 队列溢出等情况下仍能通过 snapshot/diff 保证最终一致。
...
- Linux 对 realtime source 使用 fanotify 监听源目录事件，事件作为触发提示落盘持久化。
...
- 当前开发阶段禁用所有真实 sync 执行，只保留 UI、配置保存、状态展示和 watcher 相关验证。
- 当前 Web/Tauri 的 sync API 必须直接返回“开发阶段禁用”，不能触发文件复制、删除、snapshot 清理或 cycle 推进。
- 目标设计恢复同步后，手动同步语义是“为选定 source 创建一个新的 cycle/snapshot，并让目标 dst 追赶该 cycle”。
...
- 当前开发阶段禁用所有 sync 执行，只调整 UI 和配置保存。
- 后续如恢复手动同步，语义应是“为选定 source 创建一个新的 cycle/snapshot，并让目标 dst 追赶该 cycle”，而不是直接复制当前正在变化的目录。
...
- 当前开发阶段禁用手动同步入口，避免在 UI 调整期触发真实文件操作。
