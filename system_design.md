...
- 嵌套挂载守卫（W6）：解析 source/dst 的 dataset 时读取 `/proc/self/mounts`，若树内有外来挂载点（嵌套 dataset、bind mount 等，`.zfs` 快照自动挂载除外），则拒绝对该树使用 snapshot/diff——嵌套内容不属于本 dataset，snapshot 读视图会把整棵嵌套子树当成"已删除"，`zfs diff` 也永远看不到其变化。Auto 后端此时降级 manifest 活树读并告警；显式 zfs 后端则报错。
...
- ZFS 后端的 reconcile/Full/zfs-diff 增量路径的 copy/verify source view 必须来自只读 snapshot 路径。**事件积压路径（realtime 与 daily/weekly 的 event-path 增量）按设计读 live source**（见 §8），并发写入由"读后 fingerprint 复核 + source_changing 黄色容忍 + 下轮收敛"守卫链兜底；"增长中文件不会读到半截"的保证只对 snapshot 路径成立。
- 判等语义分两档：全树 quick-check（walk Full/Compare）使用 `modify_window_secs`（默认 1s）容差吸收跨平台时间戳粒度差；**携带变化证据的路径（watcher 事件、zfs diff 输出）使用精确 mtime 比较**（仅容忍 Windows FILETIME 的 100ns 截断），否则同尺寸亚秒改写会被容差吞掉并永久漂移。双方都带 hash 时 hash 优先（dst 侧独有的 diff 路径强制补哈希，防 mtime 复原型改写）。内容一致但权限位不同的文件按 metadata 差异报告并以 chmod 原地修复，不重传内容。
...
- mirror 删除统一进入 `.auto_sync_trash/<cycle>/`；rename 失败只对跨设备（EXDEV，copy+delete 进回收站）和已消失（NotFound）两类安全错误降级，其余失败向上报错，绝不静默退化为递归硬删。回收站按 `sync.trash_keep_days`（默认 30 天，0=永久保留）回收过期 cycle 目录；`.auto_sync_tmp` 中非当前 cycle 的残留 7 天后清扫。`.auto_sync_*` 内部目录永不进入 zfs diff 并集、按路径快照或 mirror 删除集。
- dst 基线快照（dstbase）刷新前用 `zfs diff 旧基线→新基线` 交叉核对本轮触碰集：出现本轮未写过的非目录变化说明有外部写入者，刷新降级为 Retain，避免把漂移烘进基线。同名快照的创建时间早于其 cycle 开始时（旧数据库世代的遗留）会被销毁重建，防止陈旧快照被当作本轮读视图。
- peer/transfer API 可用 `app.peer_token`（各机器配置同一值）鉴权：设置后 `/api/transfer/*` 与 delegated 配置推送要求匹配请求头；为空保持内网信任模式。请求体上限 1 GiB。
- 性能形态（2026-07-03）：本地 walk Full 与 Compare、跨机 Full 的两棵树都并行扫描；本地拷贝统一走介质感知 worker 池（dst 检出 HDD 时单 worker 顺序写，flash/未知保持并行）；树扫描每条目只做一次 lstat；zfs diff 并集中 `M` 目录只取目录条目本身（变化的子项各有自己的 diff 行），`+`/`-`/`R` 保持整树递归，且递归祖先已覆盖的子路径不再重复走树；跨机整树快照走 `/api/transfer/snapshot-stream` NDJSON 流式端点（服务端边走树边发、请求端逐行解析，双端都不再缓冲整份 JSON；末行 `__status__` 决定成败，对无此端点的旧对端自动回退缓冲式接口）。
- 跨机文件传输形态（2026-07-03 晚）：≤256KiB 的非 delta 小文件按批打包进单个 `/api/transfer/put-files-batch` 请求（≤200 个 / ≤8MiB，帧格式 `<json头>\n<原始字节>`），接收端全部写入 tmp 后成批 fsync（ZFS 上连续 fsync 合并为约一次 ZIL 刷盘）再统一 rename 发布、父目录去重 fsync——消灭小文件"每文件一次往返 + 一次同步刷盘"的吞吐上限；>16MiB 大文件的余量（断点 offset 起）以单个定长请求体经 `/api/transfer/put-file-stream` 流式发送，接收端边收边写（不再每 16MiB 一个请求体驻留内存，该路由豁免 1GiB body 上限），完整性仍由 finish-file 的全文件 blake3 把关；两条新路径对无端点的旧对端分别回退逐文件 put-file 与分块循环。历史上只写不读的 `path_snapshot` 表已删除（建库时 DROP，老库一次性回收）。
- 每目的地暂停/继续（2026-07-03 晚）：`DestinationConfig.paused` 配置字段（随 delegated 推送同步到源机）。暂停 = 取消该目的地当前任务且调度器不再派发新工作——pending target 保留、状态黄色 `paused`；继续 = 调度器自动从暂停处接续（首次全量等被打断的传输无需重新发起）。手动 Sync 对暂停目的地明确拒绝，Compare（只读）不受影响。UI 上是 info 按钮左侧的 ⏸/▶ 切换钮；任务运行期间删除按钮禁用。
